...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        for dst in ["10:10:10:10:10:11", "10:10:10:10:10:12", "10:10:10:10:10:13", "10:10:10:10:10:14"]:
    ...
    def add_drop(self, datapath, priority=10, in_port=3):
        # not in use
        return
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
        ...
        self.logger.info("Switch {} DROP! priority: {} match: {}, actions: {}".format(datapath.id, priority, match, actions))
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,
                                             actions)]
        mod = parser.OFPFlowMod(datapath=datapath, priority=priority,
                                match=match, instructions=inst)
        datapath.send_msg(mod)
        self.logger.info("LEAVING DROP CLEANLY")
        ...
        for p in pkt.protocols:
            try:
                if p.protocol_name and p.src and p.dst:
                    self.logger.info("{} Packet from: {} | to {}".format(p.protocol_name, p.src, p.dst))
                elif p.protocol_name:
